...
describe('RegionComponent', () => {
  const routeMock = (regionName: string) => {
    return { snapshot: { paramMap: convertToParamMap({ 'name': regionName }) } } as unknown as ActivatedRoute;
  };
  const routes: Routes = [
  ...
  beforeEach(async () => {
    regionServiceSpy = jasmine.createSpyObj<RegionService>('RegionService', ['getRegion'], { getRegion: (regionName: string) => of([macedonia]) });
